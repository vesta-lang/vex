 ...
 * bien formada que apunta a cualquier site, y quien la sigue es el
...
int main() {
    ...
    // --- Lo que escribe --------------------------------------------------
    {
        ...
        CHECK(apply_reloc(site, BASE + 0x10, BASE + 0x1234, AOT_RELOC_RVA32,
                          BASE) == 1);
        CHECK(rd32(site) == 0x1234u);
        // Y no toca lo de al lado: son cuatro bytes, no ocho.
        CHECK(site[4] == 0xCC);

        // El propio comienzo de la imagen es el RVA cero, que es legitimo.
        CHECK(apply_reloc(site, BASE, BASE, AOT_RELOC_RVA32, BASE) == 1);
        CHECK(rd32(site) == 0u);

        // El ultimo que cabe: 4 GiB menos uno.
        CHECK(apply_reloc(site, BASE, BASE + 0xFFFFFFFFull, AOT_RELOC_RVA32,
                          BASE) == 1);
        CHECK(rd32(site) == 0xFFFFFFFFu);

        /* No depende de la VA donde se escribe, a diferencia de REL32.  Se
         * comprueba porque es la confusion natural entre las dos: un RVA es
         * relativo a la IMAGEN, no a donde se escribe. */
        CHECK(apply_reloc(site, BASE + 0x900000, BASE + 0x40, AOT_RELOC_RVA32,
                          BASE) == 1);
        CHECK(rd32(site) == 0x40u);
    }

    // --- Lo que rechaza, que es el motivo del fichero --------------------
    {
        uint8_t site[8];
        const uint8_t untouched = 0x5A;
        ...
        std::memset(site, untouched, sizeof(site));
        CHECK(apply_reloc(site, BASE, BASE - 1, AOT_RELOC_RVA32, BASE) == 0);
        CHECK(site[0] == untouched); // y no escribio nada

        // Mas de 4 GiB por encima: no cabe en el campo.
        std::memset(site, untouched, sizeof(site));
        CHECK(apply_reloc(site, BASE, BASE + 0x100000000ull, AOT_RELOC_RVA32,
                          BASE) == 0);
        CHECK(site[0] == untouched);
        ...
        std::memset(site, untouched, sizeof(site));
        CHECK(apply_reloc(site, BASE, BASE + 0x10, AOT_RELOC_RVA32,
                          AOT_NO_IMAGE_BASE) == 0);
        CHECK(site[0] == untouched);
    }
    ...
    // --- Que no se haya roto ninguna de las otras ------------------------
    {
        ...
        CHECK(apply_reloc(site, 0x1000, 0x2000, AOT_RELOC_REL32,
                          AOT_NO_IMAGE_BASE) == 1);
        CHECK(rd32(site) == 0x2000u - (0x1000u + 4u));

        std::memset(site, 0, sizeof(site));
        CHECK(apply_reloc(site, 0x1000, 0xABCD, AOT_RELOC_IMM32, BASE) == 1);
        CHECK(rd32(site) == 0xABCDu); // el valor entero, sin restar la base

        std::memset(site, 0, sizeof(site));
        CHECK(apply_reloc(site, 0x1000, BASE + 8, AOT_RELOC_ABS64, BASE) == 1);
        CHECK(rd32(site) == (uint32_t)((BASE + 8) & 0xFFFFFFFFull));
        CHECK(rd32(site + 4) == (uint32_t)((BASE + 8) >> 32));

        // Y un kind que no existe sigue devolviendo 0.
        CHECK(apply_reloc(site, 0, 0, 99, BASE) == 0);
    }
