...
const CatEntry kEntries[] = {
    ...
    {"VXT009", {"'{0}' is marked '{1}' on a value ('{2}'): writing a copy is not visible from outside, so on a parameter this is the output-parameter form -- the caller yields a slot and the callee writes it -- and that is not wired yet; write it as a pointer ('{2}*') meanwhile", "'{0}' lleva '{1}' sobre un valor ('{2}'): escribir una copia no se ve desde fuera, asi que en un parametro esto es la forma de SALIDA -- quien llama cede un hueco y el llamado lo escribe -- y todavia no esta conectada; escribelo como puntero ('{2}*') mientras tanto"}},
